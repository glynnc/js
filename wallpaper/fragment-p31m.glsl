...
void main()
{
    ...
    if (which == 0) {
        ...
        float nv = 1.0-v;
        uv = (u < nv)
            ? ((2.0*u < nv) ? vec2(v+u,u) : vec2(1.0-u,1.0-u-v))
            : ((u > 2.0*nv) ? vec2(1.0-v,2.0-u-v) : vec2(v+u-1.0,v));
    }
    else /* which == 2 */ {
        float nu = 1.0-u;
        uv = (nu > v)
            ? ((nu > 2.0*v) ? vec2(v+u,v) : vec2(1.0-v,1.0-u-v))
            : ((2.0*nu < v) ? vec2(1.0-u,2.0-u-v) : vec2(u+v-1.0,u));
    }
